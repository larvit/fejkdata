...
def write(path, columns, rows):
    """Write the rows as a TSV; the table needs a row, and every cell must be non-empty and free of tabs, newlines and braces."""
    lines = ["\t".join(columns)]
    for row in rows:
    ...
    if len(lines) < 2:
        raise ValueError(f"{path}: has no rows below its header; a table is at least two rows")
