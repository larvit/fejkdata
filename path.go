...
// the walk draws one, or stops; level runs at each template a
// segment descends into; leaf runs where the tail ends; pins is where the walk pins
// the table rows it selects and draws, nil skipping tables. A nil action is skipped.
// A render's walk sets only pins, so it allocates nothing.
...
// table by walkTable. s draws what a render picks, nil in a walk that only proves.
// A missing segment is an error, so no walk reaches past what the data holds.
...
func walkTable(s *session, t *table, tail []string, w pathWalk, descended bool) (node, error) {
	...
	if w.pins != nil {
		if err := readRow(w.pins, s, t, sel, (descended || len(tail) > 0) && !hasSelector(tail)); err != nil {
			return nil, err
		}
	}
	switch {
...
// readRow pins the row a path reads of t: the one its selector names, or, where s
// draws, one drawn where the path reads into the table.
func readRow(p *pinSet, s *session, t *table, sel string, draw bool) error {
	if sel != "" {
