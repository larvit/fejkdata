...
func (p *pinSet) excludes(cell *template) bool {
	if r, in := p.pinned(cell.cellOf); in && r != cell.cellRow {
		return true
	}
	out := false
	p.each(func(t *table, r int) {
		out = out || (cell.cellOf.descends(t) && !cell.cellOf.under(cell.cellRow, t, r))
	})
	return out
}
