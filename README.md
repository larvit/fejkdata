...
hold.go         the hold: one draw per expansion for paths and operands, the rows it pins, and its fences
draw.go         one reference draw per render and group: draw sets, the group option, and its fence
family.go       a family of linked tables: what a path reads of one, and the fence over paths into one family
