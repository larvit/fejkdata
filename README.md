...
- **`hold` names what a draw is kept in, `draw` the draw itself.** Goal 2 wants a name
  to reach one unit, so a unit takes the stem of what it is, and a file the stem of the
  units it holds.
- **A function may not spell a method; two types may.** A call writes a method with its
  receiver and a function bare, so one name on both greps as one unit, which is the
  reaching cost goal 2 counts, while the receiver before a shared method name says
  which type answers. `TestNoFunctionSpellsAMethod` holds it. A type is outside the
  rule: `renderScope.hold` answers with a `*hold`, so the two spell one unit, which no
  test can judge.
...
pinset.go       the rows a draw pins, one model for the render and the fences replaying its reads
...
family.go       a family of linked tables: what a reference path reads of one, the rows it excludes, and the fences over paths into it
